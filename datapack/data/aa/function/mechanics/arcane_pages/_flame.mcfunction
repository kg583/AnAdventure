...
execute as @a[advancements={aa:mechanics/enchantment/flame=true}] at @s if block ~ ~ ~ minecraft:lava if block ~ ~1 ~ minecraft:lava if items entity @s weapon.offhand minecraft:enchanted_book[minecraft:custom_data~{enchantment:"minecraft:flame"}] run function aa:mechanics/arcane_pages/enchant {slot:"offhand",enchantment:"minecraft:flame"}
