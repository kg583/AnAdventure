...
REPLS = {
    ...
    # Formatting codes are broken in language files
    r"\?\?(.*?)\?\?": r"\1",
    r"\*\*(.*?)\*\*": r"\1",
    r"~~(.*?)~~": r"\1",
    r"__(.*?)__": r"\1",
...
if __name__ == "__main__":
    for journal in Path("./scripts/journal/data/").rglob("*.md"):
        ...
        number = int(part.split(".", maxsplit=1)[0])
        ...
        modifier["functions"][0]["title"] = f"Journal - Part {ROMAN[number]}"
        ...
        modifier["functions"][2]["components"]["minecraft:custom_name"]["translate"] = f"item.aa.journal.{number}"
